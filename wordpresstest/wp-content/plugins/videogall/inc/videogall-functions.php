...
function videogall_get_videos_by_categories($filter_value = "", $order_value = "") {
    global $wpdb;
    $table = $wpdb->prefix."videogall_videos";
    if(trim($order_value) != "") $order_value = " ORDER BY ".$order_value;
    if($filter_value != "")
        return $wpdb->get_results("SELECT * FROM $table WHERE category in ($filter_value)$order_value",ARRAY_A);
    else
