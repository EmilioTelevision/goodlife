...
function videogall_enqueue_scripts() {
    ...
    wp_enqueue_script('videogall-fancybox-js', $videogall_url.'inc/fancybox/jquery.fancybox-1.3.4.pack.js', 'jquery');
...
function videogall_shadowbox_init() {
    ...
    echo '<script type="text/javascript">
        jQuery(document).ready(function() {
            jQuery("a[rel=\"videogall\"]").fancybox({
                "speedIn": 600, 
		"speedOut": 200,
                "showNavArrows": false,
                "overlayOpacity": 0.4,
...
function videogall_display_videos($category_filter = "", $isWidget = false, $number_of_videos = "") {
    ...
    $videos = videogall_get_videos_by_categories($filter_value,$order_filter);
    $output = '';
    $page_count = 1;
    $video_count = 1;
    ...
    foreach($videos as $entry) {
        ...
        $output .= "\t\t".'<img class="videogall-thumb" src="'.$entry['thumbnail'].'"/>'."\n";
        $output .= "\t".'</a>'."\n";

        if($entry['caption'] != "")
            $output .= "\t".'<p class="videogall-caption">'.$entry['caption'].'</p>'."\n";

        if($entry['description'] != "" and !$isWidget)
            $output .= "\t".'<p class="videogall-description">'.$entry['description'].'</p>'."\n";
    ...
    if($output != "") {
        $output = '<div id="videgoall-container">'.$output.'</div>'."\n";
    }
...
function videogall_additional_styling() {
    ...
    $output .= "\t".'.videogall-item { width: '.(floor(100/$videogall_options['number_of_columns']) - 4).'%; }'."\n";
